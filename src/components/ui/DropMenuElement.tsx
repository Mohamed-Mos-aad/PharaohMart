...
// ** Style
import style from "../../style/components/ui/dropMenuElement.module.css";
// ** Hooks && Tools
...
export default function DropMenuElement({
  title,
  selections,
  onSelect,
}: IDropMenuElement) {
  ...
  const selectionsRender = selections.map((selection) => (
    <li
      key={selection.title}
      onClick={() => {
        selectHandler(selection.id, selection.title);
      }}
    >
      {selection.title}
    </li>
  ...
  return (
    <>
      <div
        className={`${style.drop_menu} ${dropMenuOpened ? style.opened : ""}`}
        onClick={dropMenuToggleHandler}
      >
        <h2>{selected}</h2>
