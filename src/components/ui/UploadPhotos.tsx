...
// ** Style
import style from "../../style/components/ui/uploadPhotosElement.module.css";
// ** Hooks && Tools
  ...
  const renderUploadedIamgesRender = files.map((file, index) => {
    return (
      <div className={style.product_photo} key={file.name + index}>
        ...
        <span
          onClick={() => {
            removeUploadedPhotoHandler(file.id!);
          }}
        >
  ...
  return (
    <>
      <div
        ...
        style={
          quantityCompleted ? { cursor: "no-drop" } : { cursor: "default" }
        }
      >
        {files.length < 1 ? (
          <>
          ...
          <div
            className={style.uploaded_images}
        ...
        <label
          htmlFor="uploadPhoto"
          style={
            quantityCompleted ? { cursor: "no-drop" } : { cursor: "pointer" }
          }
        >
          Upload
        </label>
        <input
          ...
          id="uploadPhoto"
          hidden
          multiple={quantity > 1 ? true : false}
