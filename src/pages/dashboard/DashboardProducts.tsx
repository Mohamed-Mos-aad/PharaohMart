...
export default function DashboardProducts() {
  ...
  const applyFiltersAndSearch = () => {
    const updatedData = productsData.filter((item) => {
      ...
      const categoryMatch =
        filter.category === "All" ||
        item.categories.some((c) => c.title === filter.category);
      const statusMatch =
        filter.statu === "All" || item.isPublished === filter.statu;
    ...
    () =>
      displayedData.map((item) => (
        <tr key={item.documentId}>
          ...
          <td>
            {(item.categories[0]?.title || "No Category") +
              (item.categories[1] ? " , " + item.categories[1].title : "")}
          </td>
          <td>
            <div className={style.product_options}>
              <img src={editeIcon} alt="edit icon" />
              <img
                src={item.isPublished ? pauseIcon : playIcon}
                alt={item.isPublished ? "pause icon" : "play icon"}
                onClick={() => {
                  changeProductStateHandler(item);
                }}
              />
        ...
        <div className={style.dashboard_products}>
          ...
          <div className={style.filter_options}>
            <DropMenuElement
              ...
              onSelect={(prev) => {
                setFilter((filters) => ({ ...filters, category: prev.title }));
              }}
            />
            <DropMenuElement
              title="Stock Status"
              selections={PRODUCT_STATUS}
              onSelect={(prev) => {
                setFilter((filters) => ({
                  ...filters,
                  statu: prev.id === null ? "All" : prev.id === 0,
                }));
              }}
            />
          </div>
